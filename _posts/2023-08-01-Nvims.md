...
`-S file`
: Executes `Vimscript` or `Lua` in `file` after the first file has been read. If no `-S` argument is provided then `~/.config/lazyman/Lazyman/overrides.lua` is used if not empty.
