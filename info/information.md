...
Each supported Lazyman Neovim configuration has an information document
auto-generated by the `scripts/information.sh` script. This script calls
[scripts/keymaps.sh](https://lazyman.dev/info/keymaps.html) to generate
tables of keymaps defined by each configuration.
...
#
# Generated information documents are stored in ~/.config/lazyman/Lazyman/info/
...
usage() {
  ...
  printf "\n\t-d indicates debug mode, leave generated Lua in .config/lazyman/Lazyman/tmp/"
...
get_plugins() {
  ...
  plugman="$3"
  confdir=
  if [ -d "${HOME}/.config/lazyman/${nvimdir}" ]
  then
    confdir="${HOME}/.config/lazyman/${nvimdir}"
  else
...
make_info() {
  ...
  echo "- Installation location:  **\`~/.config/lazyman/${nvimconf}\`**" >> "${OUTF}"
  echo "" >> "${OUTF}"
  [ "${GH_URL}" ] && {
